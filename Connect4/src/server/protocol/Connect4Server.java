...
public interface Connect4Server {
	 ...
	 * @param assignedUserID this is the unique identifier the server assigned to the client 
	 * who receives this WELCOME command.
	 * @param allowedThinkTime the time the client has to send a valid response to the server in 
	 * milliseconds. If the client does not respond in time the server will decide what to do next 
	 * (this depends on the implementing server). Note that the allowed think time can be dependent 
	 * on whether the client is playing with an AI or as a human, which was reported in the 
	 * preceding HELLO command.
	 * @param capabilities represents a vector with flags for the capabilities that are implemented 
	 * in the server. See also the specification
	 * for cmdHello(...) in Connect4Client.java.
	 */
	public void cmdWelcome(int assignedUserID, long allowedThinkTime, int capabilities);
	
	/**
	 * Signal the client that a new game is started (i.e. an opponent has been found). All game 
	 * settings including the id and username of the opponent are send along with this.
	 ...
	 * @param playerWhoHasNextTurnID the ID of the player who has the first turn and who should
	...
	/**
	 * Tell the client that a valid move has been submitted to the server. This could either be the 
	 * move the receiving client has made himself,
	...
	/**
	 * Tell the client his last command was either not recognized by the server or was an invalid 
	 * action.
	 ...
	 * @param reason a message generated by the server stating the reason why the player left 
	 * (e.g. kicked)
	 */
